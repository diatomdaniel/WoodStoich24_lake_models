...
trait.var.plot <- trait.var.combined %>%
  ggplot() + 
  geom_point(aes(trait.value, log(GPP), col = Pin), size = 2) + 
...
trait.var.cn <- trait.var.combined %>%
  filter(model == "dynamic") %>%
  ggplot() + 
  geom_point(aes(trait.value, 1/QN1, col = Pin), size = 2) + 
...
trait.var.cp <- trait.var.combined %>%
  filter(model == "dynamic") %>%
  ggplot() + 
  geom_point(aes(trait.value, 1/QP1, col = Pin), size = 2) + 
  ggh4x::facet_grid2(.~trait,  scales = "free", independent = "all") + 
  scale_color_viridis_c() + 
  labs(x = NULL,
       y =  "Cell C:P (mass)",
...
trait.var.np <- trait.var.combined %>%
  filter(model == "dynamic") %>%
  ggplot() + 
  geom_point(aes(trait.value, QN1/QP1, col = Pin), size = 2) + 
  ggh4x::facet_grid2(.~trait,  scales = "free", independent = "all") + 
  scale_color_viridis_c() + 
  labs(x = "Trait value",
       y =  "Cell N:P (mass)",
