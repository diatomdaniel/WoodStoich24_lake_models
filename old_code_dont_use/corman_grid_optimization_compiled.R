...
################################################################################
### Compare optimal parameters from Edwards et al data set
# load overview of trait data and combine with "optimal" traits
freshwater.traits <- read_csv("data4input/overview_freshwater_traits.csv")
# fix names
freshwater.traits <- freshwater.traits %>%
  mutate(
    Trait = ifelse(Trait == "K_P_mgL", "KP", Trait),
    Trait = ifelse(Trait == "K_N_mgL", "KP", Trait),
    Trait = ifelse(Trait == "Qmin_mg_P_mg_C", "QminP", Trait),
    Trait = ifelse(Trait == "Qmin_mg_N_mg_C", "QminN", Trait),
    Trait = ifelse(Trait == "Vmax_mg_P_mg_C", "VmaxN", Trait),
    Trait = ifelse(Trait == "Vmax_mg_N_mg_C", "VmaxP", Trait),
    taxon = ifelse(taxon == "diatom", "diatoms", taxon),
    taxon = ifelse(taxon == "green", "greens", taxon),
    taxon = ifelse(taxon == "cyano", "cyanos", taxon),
    # some nr. editing
    Trait.Value = ifelse(Trait == "KP", Trait.Value*1000, Trait.Value),
    Trait.Value = ifelse(Trait == "KN", Trait.Value*1000, Trait.Value),
    Median.Trait.Value = ifelse(Trait == "KP", Median.Trait.Value*1000, Median.Trait.Value),
    Median.Trait.Value = ifelse(Trait == "KN", Median.Trait.Value*1000, Median.Trait.Value)
    )

# create data frame with optimal traits
opt.traits <-  bind_rows(
  static.top10 %>% mutate(model = "static"),
  dynamic.top10 %>% mutate(model = "dynamic")) %>% 
  mutate(model = factor(model, levels = c("static", "dynamic"))) %>%
  gather("Trait", "Model.Trait.value", -model, -RMSE, -MAE, -NSE) %>%
  # new names
  mutate(
    Trait = ifelse(Trait == "minQP1", "minQP", Trait),
    Trait = ifelse(Trait == "minQN1", "minQN", Trait),
    Trait = ifelse(Trait == "KN1", "KN", Trait),
    Trait = ifelse(Trait == "KP1", "KN", Trait),
    Trait = ifelse(Trait == "upN1", "VmaxN", Trait),
    Trait = ifelse(Trait == "upP1", "VmaxP", Trait),
    Trait = ifelse(Trait == "umax1", "umax", Trait)
    )

# merge
model.data.traits <- merge(freshwater.traits.traits, opt.traits, by = "Trait")
  

freshwater.traits %>%
  
  ggplot(aes(model, Trait.Value, fill = taxon.new)) + 
  geom_violin() + 
  facet_grid(Trait~.)
