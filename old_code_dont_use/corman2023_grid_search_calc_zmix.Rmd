...
c2 <- corman2 %>%
  ...
  geom_vline(xintercept = quantile(corman2$GPP, c(0.0275, 0.5, 0.975)), 
             col = "red", lty = "dashed") + 
  labs(x = expression("Std.dev monthly GPP mg C L"^-1 ~ "day"^-1), 
       y = "Density")
...
c1 + c2 + c3 + c4 + plot_layout(ncol = 4)
...
#select best parameters
# get best 10 percentile of runs
quantile(grid.new.rmse$RMSE, 0.1)
best.parm <- grid.new.rmse[grid.new.rmse$RMSE < quantile(grid.new.rmse$RMSE, 0.1),  ]
nrow(best.parm)
hist(grid.new.rmse$RMSE[grid.new.rmse$RMSE < quantile(grid.new.rmse$RMSE, 0.1)])
```
...
```{r rerun MM model with the best parameters}

# create grid with best parameter values and run it
grid.best <- best.parm %>% 
  slice(rep(1:n(), each = nrow(corman2)))
# add measured data
grid.best$SA <- rep(corman2$SA, nrow(best.parm))
grid.best$z <- rep(corman2$z, nrow(best.parm))
grid.best$DOC_mgL <- rep(corman2$DOC_mgL, nrow(best.parm))
grid.best$GPP <- rep(corman2$GPP, nrow(best.parm))
grid.best$GPP_sd - rep(corman2$GPP_sd, nrow(best.parm))
grid.best$Lake <- rep(corman2$Lake, nrow(best.parm))
grid.best$TN <- rep(corman2$TN, nrow(best.parm))
grid.best$TP <- rep(corman2$TP, nrow(best.parm))


best.runs <- lapply(1:nrow(grid.best), function(i){
  # indexing
  params.mich["SA"] = pull(grid.best[i, "SA"])
  params.mich["DOC"] =  pull(grid.best[i, "DOC_mgL"])
  params.mich["z"] =  pull(grid.best[i, "z"])
  ...
  params.mich["Pin"] = pull(grid.best[i, "TP"])
  params.mich["Nin"] = pull(grid.best[i, "TN"])
  # starting values
  y <- c("A1" = 100, "P" = pull(grid.best[i, "TP"]), "N" = pull(grid.best[i, "TN"]))
  run <- ode(y, times, parms = params.mich, func = mich.single)
  return(run[max(times),])
})
# convert to df
best.runs <- do.call(rbind, best.runs)
best.runs <- as_data_frame(best.runs)
best.runs <- rename(best.runs, est_GPP = GPP)
# combine with original df
best.runs$GPP <-  rep(corman2$GPP, nrow(best.parm))
best.runs$GPP_sd <-  rep(corman2$GPP_sd, nrow(best.parm))
best.runs$Lake <-  rep(corman2$Lake, nrow(best.parm))
best.runs$Year <-  rep(corman2$Year, nrow(best.parm))
best.runs$Month <-  rep(corman2$Month, nrow(best.parm))
# calculate medians and standard devs.

```

figures show (a) modelled vs. measured GPP, (b) modelled GPP vs TP loads, (c) modelled GPP vs TN loads; dashed line shows 1:1 relationship. Note the log10 axes!

modelled vs. measured GPP has a an Rsquated of 0.27 with p<0.001 (log10(x + 1e3) transformed!)
...
best.run.plt1 <- best.run %>%
  ...
  geom_smooth(aes(GPP, est_GPP), method = "lm") + 
  labs(x = expression("Median monthly measured GPP mg C L"^-3 ), 
       y = expression("Median monthly modelled GPP mg C m"^-3)) + 
...
best.run.plt2 <- best.run %>%
  ...
  geom_abline(slope = 1, intercept = 0, lty = "dashed", lwd = 1) + 
  labs(y = expression("Median monthly modelled GPP mg C L"^-3 ), 
...
best.run.plt3 <- best.run %>%
  ...
  geom_abline(slope = 1, intercept = 0, lty = "dashed", lwd = 1) + 
  labs(y = expression("Median monthly modelled GPP mg C L"^-3 ), 
       x = expression("Median monthly stream TN mg m"^-3)) + 
  scale_y_log10() + scale_x_log10()

best.run.plt1 + best.run.plt2 + best.run.plt3 + plot_layout(ncol = 3)

# some stats for text
summary(lm(log10(est_GPP + 1e-3) ~ log10(GPP + 1e-3), data =best.run))
#cor.test(best.run$A1, best.run$GPP)

```
ignore the Droop model output for now, will run overnight!
...
#which.min(grid.droop.new.rmse$RMSE)
# select best parameters
# lower 2.5th percentiles of runs
quantile(grid.droop.new.rmse$RMSE, 0.0275)
best.parm.droop <- grid.droop.new.rmse[grid.droop.new.rmse$RMSE < quantile(grid.droop.new.rmse$RMSE, 0.0275),  ]
nrow(best.parm.droop)
```
...
```{r rerun droop  model with the best parameters}

# create grid with best parameter values and run it
grid.droop.best <- best.parm.droop %>% 
  slice(rep(1:n(), each = nrow(corman2)))
# add measured data
grid.droop.best$SA <- rep(corman2$SA, nrow(best.parm.droop))
grid.droop.best$z <- rep(corman2$z, nrow(best.parm.droop))
grid.droop.best$DOC_mgL <- rep(corman2$DOC_mgL, nrow(best.parm.droop))
grid.droop.best$GPP <- rep(corman2$GPP, nrow(best.parm.droop))
grid.droop.best$Lake <- rep(corman2$Lake, nrow(best.parm.droop))
grid.droop.best$TN <- rep(corman2$TN, nrow(best.parm.droop))
grid.droop.best$TP <- rep(corman2$TP, nrow(best.parm.droop))


best.run.droop <- lapply(1:nrow(grid.droop.best[-160,]), function(i){
  # indexing
  print(i)
  params.droop["SA"] = pull(grid.droop.best[i, "SA"])
  params.droop["DOC"] = pull(grid.droop.best[i, "DOC_mgL"])
  params.droop["z"] = pull(grid.droop.best[i, "z"])
  params.droop["KP1"] = pull(grid.droop.best[i, "KP1"])
  params.droop["KN1"] = pull(grid.droop.best[i, "KN1"])
  params.droop["minQP1"] = pull(grid.droop.best[i, "minQP1"])
  params.droop["minQN1"] = pull(best.parm.droop[i, "minQN1"])
  params.droop["upP1"] = pull(grid.droop.best[i, "upP1"])
  params.droop["upN1"] = pull(grid.droop.best[i, "upN1"])
  params.droop["Pin"] = pull(grid.droop.best[i, "TP"])
  params.droop["Nin"] = pull(grid.droop.best[i, "TN"])
  # starting values
  y <- c("A1" = 100, "P" = pull(grid.droop.best[i, "TP"]),
         "N" = pull(grid.droop.best[i, "TN"]),
          "QP1" = 0.015,"QN1" = 0.1)
  run <- ode(y, times, parms = params.droop, func = droop.single)
  return(run[max(times),])
})
