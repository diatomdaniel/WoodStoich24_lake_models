...
(obs.plt <- corman2 %>%
  ggplot(aes(TP_in, GPP, col = TN_in/TP_in)) + 
  geom_point(size = 2) + 
...
(mod.plt <- base.predictions.corman %>%
    ggplot(aes(TP_in, est_GPP, col = TN_in/TP_in)) + 
    geom_point(size = 2) + 
...
(predicted.plt <- base.predictions.corman %>%
    ...
    labs(x = expression("Measured GPP (mg C L"^-1 ~ " day"^-1~")"), 
         y = expression("Modelled GPP (mg C L"^-1 ~ " day"^-1~")"),
         shape = NULL) + 
    scale_shape_manual(
      values = c(
        "Acton" = 0,
...
#save_plot("figures/gpp_validation_colimitation_colimiation.png", corman.validation.plt, base_height = 8, base_width = 14)
