---
title: "Competition simulations without light"
...
-   Should really rename the Michaelis Menten models to Monod model. This will create quite a bit of confusion if you don't.
...
# mich.triple 
source("models/mich_trip_noLight.R")# michaelis menten kinites, three algae species; model 2 in Carly's framework
# droop.single
source("models/droop_single.R") # droop model, one algae species; model 3 in Carly's framework
# droop.tripl
source("models/droop_trip_noLight.R") # droop model, three algae species; model 4 in Carly's framework
...
# issue occuring at row 34:35 of loads. dunno why
...
all.michaelis <- do.call(rbind, list(
  ...
  mutate(Trait = factor(Trait, 
                        levels = c("Median", "25th quantile", "75th quantile")))
# Droop
...
comp.out.michaelis <- all.michaelis %>%
  group_by(Trait, winner) %>%
  summarise(n = n(),
            perc.occurence = n*100/100) %>%
  pivot_wider(id_cols = c(winner), names_from = Trait, values_from = perc.occurence)
comp.out.michaelis
...
final.fig <- ggpubr::ggarrange(plotlist = list(droop.figure, michaelis.figure), 
                               ncol = 2, nrow = 1, align = "hv",
                               legend = "bottom")

final.fig

ggsave2("competition simulations no light.jpg", 
